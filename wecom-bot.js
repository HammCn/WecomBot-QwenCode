...
async function handleVoiceMessage(frame) {
    const body = frame.body
    const content = body.voice?.content
    if (!content) return
    const streamId = generateReqId('stream');
    await wsClient.replyStream(frame, streamId, '<think></think>', false);
    executeQwenCommand(content, frame, streamId)
...
async function handleMixedMessage(frame) {
    const body = frame.body;
    const msg_item = body.mixed?.msg_item
    if (!msg_item) return
    const streamId = generateReqId('stream');
    await wsClient.replyStream(frame, streamId, '<think></think>', false);
    let content = ""
    let paths = []
    for (const item of msg_item) {
        console.log(item)
    ...
    console.log(paths.join(",") + "，" + content)
    executeQwenCommand(paths.join(",") + "，" + content, frame, streamId)
}
...
wsClient.on('message.file', handleFileMessage)
